        ...
        <header class="sticky top-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 py-3.5 shadow-sm">
            <div class="max-w-7xl mx-auto flex items-center justify-between">
                <Link href="/" class="flex items-center space-x-3 shrink-0">
                    <div v-if="organization?.logo_path" class="h-10 w-10 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-center bg-white p-1 shrink-0">
                        <img :src="organization.logo_path" :alt="organization.name" class="h-full w-full object-contain" />
                    </div>
                    <div v-else class="h-10 w-10 rounded-xl bg-[#00b2e3] text-white flex items-center justify-center shadow-md shadow-[#00b2e3]/20 font-bold shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 hidden sm:block">Employer Hiring & Dynamic KBS Rules Portal</p>
                    </div>
                </Link>
            ...
            >
                <div v-if="mobileMenuOpen" class="md:hidden mt-3 pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
                    <button
                        @click="showJobModal = true; mobileMenuOpen = false"
